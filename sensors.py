...
class Sensors:
    def __init__(self):
        ...
        self.csm.value = True
        self.imu = adafruit_lsm9ds1.LSM9DS1_SPI(self.spi, self.csag, self.csm)
        ...
        self.filter = Mahony(acc=np.asarray(acc),
                             gyr=np.asarray(gyro),
                             #  mag=np.asarray(mag),
                             Dt=(self.last_calc - start_time) / 10.0, k_P=2.0,
                             k_I=0.1)
        # initial attitude
            ...
            while True:
                if self.imu_ready.value:
                    ...
                    self.gyro = (np.fromiter(
                        ...
                        - self.gyro_offsets)
                    #  mag = np.fromiter(self.imu.magnetic, dtype=np.double) * [
                    #  1 / 100, 1 / 100, 1 / 100]
                    #  print(acc, gyro, mag)
